...
public class Transition
{
    ...
    private Func<string> m_Destination = () => "Idle";
    ...
    public Transition(Func<string> destination, Func<bool> conditions = null)
    {
        m_Destination = destination;
        if (conditions != null) m_Conditions = conditions;
    }
...
public class ActorStateScript
{
    ...
    protected List<Transition> m_FixedTrans = null;
    ...
    protected Transform target => m_Actor.Target;

    protected Transform transform => m_Actor.transform;
    ...
    // This is called every frame, even if the state is not active
    ...
    public bool OnFixedTransition(out string next)
    {
        if (m_FixedTrans != null) foreach (var i in m_FixedTrans)
        {
            if (i.Condition(out next)) return true;
        }
        next = null;
    ...
    public ActorStateScript AddStartFuncs(List<Action> func)
    {
        if (m_StartFunc == null) m_StartFunc = new List<Action>();
    ...
    public bool CanLookPoint(Transform target) => target == null ? false : CanLookPoint(target.position);
